...
namespace LearnDocUtils
{
    public static class SinglePageToDocx
    {
            ...
            string accessToken = null, DocumentOptions options = null)
        {
            ...
            if (metadata.PageType != "conceptual")
                throw new ArgumentException("Docs URL must point to a conceptual page.");
            return await ConvertFromRepoAsync(metadata.Organization, metadata.Repository, metadata.Branch,
                metadata.ContentPath, outputFile, accessToken, options);
        }
